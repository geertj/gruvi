...
Here we import the class :class:`~gruvi.http.HttpClient` from
:mod:`gruvi.http`. This shows a few things:
...
* The ``getresponse`` call only reads the HTTP header. The HTTP body needs to
  be read separately. This means that you can use Gruvi's HTTP client to
  efficiently transfer large files.
...
Here we see that some things live in the global :mod:`gruvi` namespace as
well. The function :meth:`gruvi.get_hub` returns a reference to the Hub, and
:mod:`gruvi.util` is a module containing some utility functions.

The second import of :class:`gruvi.stream.StreamServer` imports the stream
server protocol. The stream protocol is the simplest protocol possible. It
merely interprets its underlying transport as a sequence of bytes. Being a
protocol implementation, we again see that it lives in its separate module
under the ``gruvi`` namespace.

.. literalinclude:: ../examples/echoserver1.py
   :lines: 15-23

Here we define a function that will be used by the ``StreamServer`` as the
stream handler. Some notes about this function:
...
* The handler function takes three arguments. The ``stream`` argument is an
  ...
  transport. The ``protocol`` argument is a reference to the ``StreamServer``
...
In these two lines we create the ``StreamServer`` instance, and make it listen
on the specified port. The address ``'0.0.0.0 '`` indicates the *wildcard
address* which means to listen on any interface in the system.

.. literalinclude:: ../examples/echoserver1.py
   :lines: 28-29

Here we use :func:`gruvi.get_hub` to get a reference to the Hub, and then we
switch to it using :meth:`gruvi.Hub.switch`. This does the following:

* Because the Hub wasn't used before, the first call to ``get_hub()`` will
  create it.
* When switching to the Hub, it will run its event loop as long as there is
  ...
  In this case, ``StreamServer`` has created a listening socket. This socket
  is a passive socket that will keep the event loop busy until it is explicitly
  closed. Since we never close the socket, the call to ``switch()`` would
  normally never return.
* The argument ``interrupt=True`` to ``switch()`` however tells the Hub to
  switch back to the caller when CTRL-C is pressed.
